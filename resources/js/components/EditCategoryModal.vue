    ...
    <div class="fixed inset-0 z-[300] flex items-center justify-center bg-black/50 p-4" @click.self="$emit('close')">
        <div class="bg-[var(--bg-card)] rounded-2xl p-4 sm:p-5 w-full max-w-sm shadow-2xl relative max-h-[90vh] overflow-y-auto overflow-x-hidden border border-[var(--color-border)]">
            <div class="flex justify-between items-center mb-4">
                <h2 class="text-lg font-bold text-[var(--color-text)]">Настройка категории</h2>
            ...
            <div class="space-y-4">
                <div class="min-w-0">
                    <label class="text-[10px] text-[var(--color-secondary)] uppercase font-bold px-1 tracking-wider">Название</label>
                ...
                <div class="min-w-0">
                    ...
                    <div class="flex items-center gap-3 mt-1">
                        <input v-model.number="editData.weight" type="range" min="1" max="99" class="flex-1 accent-[var(--color-text)]">
                    </div>
                </div>

                <div class="flex flex-col sm:grid sm:grid-cols-2 gap-3">
                    <div class="min-w-0">
                        <label class="text-[10px] text-[var(--color-secondary)] uppercase font-bold px-1 tracking-wider">Цвет</label>
                    ...
                    <div class="min-w-0">
                        <label class="text-[10px] text-[var(--color-secondary)] uppercase font-bold px-1 tracking-wider">Скрыто до (время)</label>
                ...
                <div class="pt-4 flex gap-2 border-t border-[var(--color-border)]">
                    <button v-if="!isNew" @click="$emit('delete', slug)" class="w-12 py-3 bg-[var(--bg-secondary)] text-red-500 rounded-xl flex items-center justify-center hover:opacity-80 transition-colors border border-[var(--color-border)]">
                    ...
                    <button @click="handleSave" class="flex-1 py-3 bg-[var(--btn-primary-bg)] text-[var(--btn-primary-text)] border border-[var(--color-border)] rounded-xl font-bold text-sm shadow-sm active:scale-[0.98] transition-all">
                        Готово
                    </button>
                </div>
            </div>
