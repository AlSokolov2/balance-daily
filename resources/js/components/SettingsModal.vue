    ...
    <div class="fixed inset-0 z-[200] flex items-center justify-center bg-black/50 p-2 sm:p-4" @click.self="$emit('close')">
        <div class="bg-[var(--bg-card)] rounded-[24px] sm:rounded-[32px] w-full max-w-[560px] h-[80vh] landscape:h-[95vh] flex flex-col overflow-hidden shadow-2xl relative border border-[var(--color-border)]">
            ...
            <div class="p-5 pb-3 shrink-0 landscape:p-3 landscape:pb-1 flex justify-between items-center">
                ...
                <div class="shrink-0 w-8 h-8 flex items-center justify-center cursor-pointer text-2xl text-[var(--color-secondary)] hover:text-[var(--color-text)] transition-colors" @click="$emit('close')">
                    &times;
                </div>
            </div>
                ...
                <div class="flex gap-1 bg-[var(--bg-secondary)]/50 p-1 rounded-xl border border-[var(--color-border)] landscape:rounded-lg overflow-x-auto scrollbar-hide">
                    <button
                        ...
                        :class="['flex-1 min-w-[70px] flex items-center justify-center gap-2 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all border-none shadow-none relative landscape:py-1 landscape:text-[8px]', 
                                 tab === tabName ? 'bg-[var(--bg-card)] text-[var(--color-primary)] shadow-sm' : 'bg-transparent text-[var(--color-secondary)] hover:text-[var(--color-text)]']"
                        @click="tab = tabName"
                    >
                        <!-- Tab Icons -->
                        <svg
                        ...
                        ><path
                            ...
                            stroke-linejoin="round"
                            stroke-width="2.5"
                            d="M19 11H5m14 0 a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"
                        /></svg>
                        <svg
                            v-if="tabName === 'sub'"
                            class="w-3.5 h-3.5"
                        ...
                        <span :class="{'hidden sm:block': tab !== tabName}">{{ $t(`settings.tabs.${tabName}`) }}</span>
                        <div v-if="tab === tabName" class="absolute -bottom-1 left-1/2 -translate-x-1/2 w-4 h-0.5 bg-[var(--color-primary)] rounded-full" />
                    </button>
                </div>
            </div>
            ...
            >
                ...
                <div v-if="tab === 'gen'" class="space-y-6 pb-2">
                    ...
                    <div class="p-4 bg-[var(--bg-secondary)] border border-[var(--color-border)] rounded-2xl flex items-center justify-between">
                        ...
                        >
                            <div
                                :class="['absolute top-1 left-1 w-4 h-4 bg-white rounded-full transition-all shadow-sm', 
                                         store.notificationsEnabled ? 'translate-x-6' : 'translate-x-0']"
                            />
                        </button>
                    </div>
                </div>
                ...
                <div v-if="tab === 'cat'" class="space-y-2 pb-2">
                    ...
                    >
                        <div class="flex items-center gap-3">
                            <div class="w-4 h-4 rounded-full shadow-inner border border-white/20" :style="{ backgroundColor: c.color }" />
                    ...
                    <div class="flex gap-2 pt-4">
                        <button class="flex-1 py-3 bg-[var(--bg-secondary)] text-[var(--color-text)] rounded-xl font-bold text-xs hover:opacity-80 transition-opacity border border-[var(--color-border)]" @click="addCategory">
                            {{ $t('settings_modal.categories.add_button') }}
                        </button>
                        <button class="flex-1 py-3 bg-[var(--btn-primary-bg)] text-[var(--btn-primary-text)] border border-[var(--color-border)] rounded-xl font-bold text-xs" @click="saveCats">
                ...
                <div v-if="tab === 'sub'" class="space-y-1">
                    <div v-for="(coeff, name) in store.subcatCoeffs" :key="name" class="flex items-center gap-3 py-3 border-b border-[var(--color-border)] last:border-0">
                        ...
                        <button class="p-2 text-[var(--color-secondary)] hover:text-red-500 rounded-lg hover:bg-red-50/10 transition-all border-none shadow-none bg-transparent" @click="deleteSubcat(name)">
                            <svg
                                class="w-4 h-4"
                                fill="none"
                                stroke="currentColor"
                    ...
                    <button class="w-full py-3.5 bg-[var(--btn-primary-bg)] text-[var(--btn-primary-text)] border border-[var(--color-border)] rounded-2xl font-bold text-sm mt-6 shadow-sm transition-all" @click="saveSubcats">
                ...
                <div v-if="tab === 'notepad'" class="h-full flex flex-col">
                    ...
                    />
                    <button class="shrink-0 w-full py-3.5 bg-[var(--btn-primary-bg)] text-[var(--btn-primary-text)] border border-[var(--color-border)] rounded-2xl font-bold text-sm shadow-sm transition-all" @click="saveNotepad">
                ...
                <div v-if="tab === 'data'" class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 gap-2">
                        <button class="w-full py-3.5 bg-[var(--bg-secondary)] text-[var(--color-text)] rounded-2xl font-bold text-sm hover:opacity-80 transition-all flex items-center justify-center gap-3 border border-[var(--color-border)]" @click="store.sync(true)">
                            <svg
                                class="w-4 h-4"
                                fill="none"
                                stroke="currentColor"
...
const saveCats = async () => {
    try {
        ...
        console.error('Save cats error:', _e);
