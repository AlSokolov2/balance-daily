...
const heatmapWeeks = computed(() => {
    ...
    startDate.setDate(now.getDate() - 90); // Show last 90 days
    
    // Find previous Monday for the start of the grid
    ...
    while (iterDate <= now) {
        ...
        currentWeek.push({
            date: dateStr,
            count: store.stats.heatmap[dateStr] || 0
        });
...
// --- Balance Logic ---
const sortedBalance = computed(() => {
    return [...(store.stats?.category_balance || [])].sort((a, b) => b.count - a.count);
});

const maxBalance = computed(() => {
    return Math.max(...(store.stats?.category_balance.map(b => b.count) || [1]));
});
